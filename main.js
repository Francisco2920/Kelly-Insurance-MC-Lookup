...
/*| Version: v1.0.1                                                    |*/
/*|====================================================================|*/
/*| Date: 11/4/2020 - Change: Fixed truck amount selectors. Array was  |*/
/*| incorrectly declared out of FOR loop, so it did not resave the new |*/
/*| truck amount when a new MCNUMBER was fed into the program.         |*/
/*|--------------------------------------------------------------------|*/
/*| Author: Jack Kelly                                                 |*/
/*| Date: 11/4/2020                                                    |*/
/*| Version: v1.0.9                                                    |*/
...
let activePendingButtonSelector = 'body > font > center:nth-child(13) > table > tbody > tr > td:nth-child(1) > form';
...
async function findDetails() {
    ...
    for (var i = 0; i < filteredMcNumbers.length; i++) {
        ...
        console.log("\nCurrent MC number: " + filteredMcNumbers[i]);
        ...
        if (captchaNotReady == false && captchaError == false) {
            ...
            if (htmlButton == true && infoTable == true) {
                ...
                try {
                    await page.waitForSelector('body > font > center:nth-child(13) > table > tbody > tr > td:nth-child(1) > form', {
                        timeout: 6000
                    });
            ...
            if (activePendingPageInfo == true) {
                ...
                if (snapshotValid == true) {
                    ...
                    if (snapshotTruckAmount != "Error. Company is no longer active or has $0 of Insurance on File." && activeStatus == true && insuranceOnFile == true) {
                        var body = ("<h3><center>Motor Carrier Details</center></h3>" + motorCarrierDetailsPageHTML + "<b><br><center><h3>Amount of trucks: " + snapshotTruckAmount + "</center></h3>" + activePendingPageInfoTableHTML + "<br><br><b><hr></b><div class=\"page-break\"></div>");
                      ...
                      html = ("<html><head><title>MC-" + mcNumber + " Carrier Report</title></head><body>" + body + "</body></html>");
                        var overviewHTML = ("<html><head><title>Overview for " + filteredMcNumbers.length + " MCNumbers - " + getDate() + "</title></head><body><style>@media print{.page-break{ display:block; page-break-before:always; }}</style><h1><center>Overview</center></h1><br>" + overviewBody + "</body></html>");
                        ...
                        console.log("\nTHERE ARE " + snapshotTruckAmount + " TRUCKS FOR MC NUMBER: " + mcNumber );
                        ...
                        mainWindow.webContents.send('progress:saved', saved, lengthTotal);
                    }
    ...
    ipcMain.on('show:results', function(e, mcNumbers) {
      printResults();
    });
}
